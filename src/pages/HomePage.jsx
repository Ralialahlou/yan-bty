...
// Scroll reveal hook — adds .visible to any [data-reveal] element when it enters viewport
function useReveal() {
  useEffect(() => {
...
export default function HomePage() {
  ...
  return (
    <main className={styles.main}>
      ...
      {/* ═══════════════════════════════════════
          HERO — Full viewport with video/cinematic background
          ═══════════════════════════════════════ */}
      <section className={styles.hero}>

        {/* Cinematic background: CSS Ken Burns crossfade (always on) */}
        ...
        {/* Optional HTML5 video overlay — client replaces src with brand video */}
        {/* To enable: uncomment and set src to your video URL (MP4 recommended) */}
        {/*
        <video
          ref={videoRef}
        ...
        */}
        ...
        <div className={styles.heroContent}>
          ...
          <h1 className={styles.heroHeadline}>
            ...
            <span className={[styles.heroLine, styles.heroLineAccent].join(' ')}>Beautiful.</span>
          ...
          <div className={styles.heroCtas}>
            ...
            <Link to="/brand/yan-one">
              <button className={styles.heroGhostBtn}>
                Explore Yan&One <ArrowRight size={16} />
              </button>
            </Link>
          </div>
      ...
      {/* ═══════════════════════════════════════
          CATEGORIES — Editorial image grid
          ═══════════════════════════════════════ */}
      ...
      {/* ═══════════════════════════════════════
          YAN & ONE — Dark editorial spotlight
          ═══════════════════════════════════════ */}
      <section className={styles.yanOneSection}>
        ...
        <div className={styles.yanOneTextCol} data-reveal>
          ...
          <Link to="/brand/yan-one">
            <button className={styles.yanOneCta}>
              Shop Yan&One <ArrowRight size={16} />
            </button>
          </Link>
        </div>

        {/* Right: horizontal scrollable product strip */}
      ...
      <section className={styles.productSection}>
        <div className="container">
          <header className={styles.secHeader} data-reveal>
            ...
            <Link to="/shop?filter=bestsellers" className={styles.secLink}>
      ...
      {/* ═══════════════════════════════════════
          EDITORIAL SPLIT — "Beauty begins..."
          ═══════════════════════════════════════ */}
      <section className={styles.editorialSplit}>
        <div className={styles.editorialImg} data-reveal>
          <img
            src="https://images.unsplash.com/photo-1614530291012-b6b3d1ca1f07?w=1200&q=85"
            alt="The Art of Moroccan Beauty"
            loading="lazy"
          />
      ...
      {/* ═══════════════════════════════════════
          NEW ARRIVALS — Full-bleed accent background
          ═══════════════════════════════════════ */}
      <section className={styles.newSection}>
        <div className="container">
          <header className={styles.secHeader} data-reveal>
            ...
            <Link to="/shop?filter=new" className={styles.secLink}>
              View All <ArrowRight size={14} />
            </Link>
          </header>
      ...
      {/* ═══════════════════════════════════════
          EDITOR'S PICKS — Dark editorial
          ═══════════════════════════════════════ */}
      <section className={styles.editorSection}>
        <div className="container">
          <header className={styles.secHeaderCenter} data-reveal>
            <p className={styles.secLabelLight}>Yan BTY Editors</p>
          ...
          <div className={styles.editorCta} data-reveal>
            <Link to="/shop?filter=editors">
              <Button variant="light" size="lg">View All Picks <ArrowRight size={14} /></Button>
            </Link>
          </div>
        </div>
      </section>
          ...
          <div className={styles.brandsTrack} data-reveal>
            {featuredBrands.map(b => (
              <Link key={b.id} to={`/brand/${b.slug}`} className={styles.brandChip}>
                <div className={styles.brandChipLogo} style={{ '--bc': b.color }}>
                  {b.name[0]}
                </div>
        ...
        <div className="container">
          ...
          <div className={styles.journalGrid} data-reveal>
            {editorialArticles.slice(0, 3).map((a, i) => (
              <Link key={a.id} to={`/journal/${a.slug}`} className={[styles.journalCard, i === 0 ? styles.journalCardFeat : ''].join(' ')}>
      ...
      <section className={styles.profileSection}>
        ...
        <div className={styles.profileText} data-reveal>
          ...
          <Link to="/account/beauty-profile">
            <Button variant="primary" size="lg">Take the Beauty Quiz <ArrowRight size={14} /></Button>
          </Link>
        </div>
      </section>

      {/* ═══════════════════════════════════════
          LOYALTY CTA — Full-bleed dark
          ═══════════════════════════════════════ */}
      <section className={styles.loyaltySection}>
        ...
        <div className="container">
          <div className={styles.loyaltyContent} data-reveal>
            ...
            <Link to="/account">
              <Button variant="accent" size="lg">Join Now — It's Free</Button>
            </Link>
          </div>
        </div>
      </section>
